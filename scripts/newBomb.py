...
class NewBombFactory(BombFactory):
    def __init__(self):
        ...
        self.santaModel=bs.getModel('santaHead')
        self.boxModel=bs.getModel('box')

        self.bonesModel =bs.getModel('bonesHead')
        self.puckModel = bs.getModel('puck')
        ...
        self.treesTexture = bs.getTexture('treesColor')
      #  self.bgTex = bs.getTexture('menuBG')
        self.testColorTexture = bs.getTexture('thePadLevelColor')
        self.lava = bs.getTexture('lava')
        self.jump = bs.getTexture('buttonJump')
        self.ouya = bs.getTexture("ouyaOButton")
        self.Shutout= bs.getTexture("achievementFootballShutout")
        self.bunny= bs.getTexture("bunnyColor")
        self.aliSplash= bs.getTexture("aliSplash")
        self.heart= bs.getTexture("heart")
        self.monkeyFaceLevelColor= bs.getTexture('monkeyFaceLevelColor')
        self.natureBackgroundColor= bs.getTexture('natureBackgroundColor')
        self.aliColor= bs.getTexture('aliColor')
        ...
        if isinstance(msg, bs.DieMessage):
            ...
            if node is not None and node.exists():
                ...
                if self.blastType == "ice" or self.blastType == "iceImpact" or self.blastType == "iceMine" or self.blastType == "stickyIce":
                    ...
                    bs.gameTimer(8000, self.portal.delete)
                elif self.blastType == "cursyBomb":
                    ...
                    node.handleMessage(bs.PowerupMessage(powerupType='weed'))
...
class NewBomb(Bomb):
    def __init__(self, position=(0, 1, 0), velocity=(0, 0, 0), bombType='normal',
        ...
        if self.bombType == 'sticky' or self.bombType == 'stickyIce' or self.bombType == 'atomBomb': self._lastStickySoundTime = 0
        ...
        if self.bombType == 'sticky' or self.bombType == 'stickyIce' or self.bombType == 'atomBomb':
        ...
        if self.bombType == 'landMine':
            ...
            if r ==1:model=factory.bombModel
            elif r ==2:model=factory.stickyIceModel
            elif r == 3:model=factory.stickyBombModel
            elif r == 4:model=factory.iceImpactModel
            elif r == 5:model=factory.spazBombModel
            elif r == 6:model = factory.bonesModel
            elif r == 7:model = factory.iceMineModel
            elif r == 8:model = factory.teleBombModel
            elif r == 9:model = factory.santaModel
            else:
                r2=random.randint(1,3)
                if r2 == 1:model =factory.boxModel
                elif r2 == 2:model =factory.tntModel
                else:model = factory.puckModel
            ...
            textures = [
                ...
                factory.tntTex,
                factory.curseBombTex,
                factory.knockBombTex,
                factory.weedbombTex,
                factory.hybridBombTex,
                factory.iceImpactTex,
                factory.blastBombTex,
                factory.boomBombTex,
          #      factory.revengeBombTex,
                factory.gluebombTex,
            #    factory.atomBombTex,
                factory.spazBombTex,
                factory.frozenBombTex,
                factory.portalBombTex,
                factory.cursyBombTex,
                factory.teleBombTex,
                factory.fireBombTex,
                factory.iceMineTex,
                factory.iceMineLitTex,
                factory.stickyIceTex,
                factory.shockWaveTex,
            #    factory.powerupTex,
           #     factory.powerupTex1,
           #     factory.powerupTex2,
            #    factory.powerupTex3,
           #     factory.powerupTex4,
            #    factory.powerupTex5,
             #   factory.powerupTex6
                factory.treesTexture,
              #  factory.bgTex,
                factory.testColorTexture,
                factory.lava,
                factory.jump,
                factory.ouya,
                factory.Shutout,
                ...
                factory.aliSplash,
                factory.heart,
                factory.monkeyFaceLevelColor,
                factory.natureBackgroundColor,
                factory.aliColor,


            ]

            def getBombTexture(textures):
                now = datetime.datetime.now()
                minute = now.minute
                index = minute % len(textures)
                return textures[index]
           # tex = random.choice(textures)
            tex = getBombTexture(textures)
          #  print tex
            nice_colors = [
                (1.0, 0.2, 0.2),
                (0.2, 0.6, 1.0),
        ...
        if self.bombType not in ('landMine', 'tnt','epicMine','powerup','iceMine'):
            ...
            bts = ('sticky','ice','revengeBomb','normal','blastBomb','hybridBomb','gluebomb','frozenBomb','cursyBomb', 'atomBomb',
            ...
            if False:#throw bombs effect
                if self.bombType == bombType:
                    def _spark():
                        bs.emitBGDynamics(position=position, velocity=(0, 7, 0),count=random.randint(100,1000),scale=0.3+random.random()*1.1,
                    ...
                    def _light():
                        ...
                        bsUtils.animate(light, "intensity", {
                            ...
                            int(s * 2000): 0.00 * iScale, int(s * 3000): 0.0})
                        def _emit():
                            bs.emitBGDynamics(position=position, velocity=(0, 1, 0), count=random.randint(100, 500),
                                              scale=0.3 + random.random() * 1.1,
                                              spread=0.2, chunkType='ice')

                        bs.gameTimer(0, _emit)
                    def _t():
                            bs.emitBGDynamics(position=position, velocity=velocity,
                                              count=int(4.0 + random.random() * 8), scale=2.8,
                                              chunkType='metal');
                            ...
                            bs.emitBGDynamics(position=position, velocity=velocity,
                                              count=int(8.0 + random.random() * 15), scale=2.8,
                                              spread=25.5, chunkType='spark');
                    _r=random.randint(1,3)
                    if _r== 1 :
                        bs.gameTimer(10, _light)  # looks better if we delay a bit
                    elif _r== 2:
                        bs.gameTimer(10, _t)
                    else:
                        bs.gameTimer(10, _spark)
            if False:#set_bomb.spikeModel
                if self.bombType == bombType:
                    m = bs.newNode('math', owner=self.node, attrs={'input1': (0, -0.03, 0), 'operation': 'add'})
                    self.node.connectAttr('position', m, 'input2')
                    self.nodeShield = bs.newNode('shield', owner=self.node, attrs={'color': ((0+random.random()*6.0),(0+random.random()*6.0),(0+random.random()*6.0)),
    ...
    def _handleDropped(self, m):
        ...
        elif self.bombType == 'sticky' or self.bombType == 'stickyIce' or self.bombType == 'atomBomb':
    ...
    def _handleImpact(self, m):
        ...
        if node is not None and node.exists():
            if (self.bombType == 'impact' and
                ...
                # Only ignore impacts on *real players*.
                # Bots/spazbots should still trigger impact detonation.
                try:
                    if isinstance(nodeDelegate, bsSpaz.PlayerSpaz):
                        return
                except Exception:
                    pass

                try:
                    if hasattr(nodeDelegate, 'getPlayer'):
                        _p = nodeDelegate.getPlayer()
