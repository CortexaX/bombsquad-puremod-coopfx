...
class Chooser(object):
    ...
    def __init__(self, vPos, player, lobby):
        ...
        texts = fire.welcome_msg
        bs.screenMessage(texts, color=((0+random.random()*1.0),(0+random.random()*1.0),(0+random.random()*1.0)), clients=[clID], transient=True)
       
        profile = self._player.get_account_id()
        #
        profile_name = self._player.getName()
        #
        numbers = str(random.randrange(000000,999999))
        
        def _kicker(client_id, authentication):
            def kick():
                bsInternal._disconnectClient(client_id)
                if authentication:
                    #bs.screenMessage('Server Kicking for Authentication Sharing ---> '+profile,color=(1,1,1), clients=[client_id], transient=True)
                    pass
                    # for authentication
                else:
                    #bs.screenMessage('Server Kicking for Verification Sharing ---> '+profile,color=(1,1,1), clients=[client_id], transient=True)
                    pass
                    # for verification
            bs.gameTimer(1000,bs.Call(kick))
        
        def _log_authentication(account, name):
            import datetime
            now = datetime.datetime.now()
            f = open("auth_verf.txt", "a")
            f.write('Authentication Profile Denied: Profile Sharing ---> '+account+'\n')
            f.write('Server Kicking: Profile Sharing ---> '+name+'\n')
            f.write('Time Joined ---> '+now.strftime("%Y-%m-%d %H:%M:%S"+'\n'))
            f.write('-----------------------------------------------------------------\n')
            f.close()
            
        def _log_verification(account, name):
            import datetime
            now = datetime.datetime.now()
            f = open("auth_verf.txt", "a")
            f.write('Verification Profile Denied: Profile Sharing ---> '+account+'\n')
            f.write('Server Kicking: Profile Sharing ---> '+name+'\n')
            f.write('Time Joined ---> '+now.strftime("%Y-%m-%d %H:%M:%S"+'\n'))
            f.write('-----------------------------------------------------------------\n')
            f.close()

        def _add_profile_authentication(account):
            MID.authentication[account] = numbers
            mid_storage.save(MID, {'authentication': MID.authentication})
            print('Authentication Profile has been added! ---> '+account)
            #bs.screenMessage('Authentication Profile has been added!'+str(profile),color=(1,1,1), clients=[clID], transient=True)
            
        def _add_profile_verification(account, add):
            if add:
                MID.verification.append(account)
            else:
                if account in MID.verification:
                    MID.verification.remove(account)
            mid_storage.save(MID, {'verification': MID.verification})
            print('Verification Profile has been added! ---> '+account)
            #bs.screenMessage('Verification Profile has been added! ---> '+profile,color=(1,1,1), clients=[clID], transient=True)
            if not add:
                #bs.screenMessage('Verification Profile has been removed! ---> '+profile,color=(1,1,1), clients=[clID], transient=True)
                pass
            
        def _remove_profile_authentication(account):
            if account in MID.authentication:
                del MID.authentication[account]
            mid_storage.save(MID, {'authentication': MID.authentication})
            print('Authentication Profile has been Removed! ---> '+account)
            bs.screenMessage('Authentication Profile has been removed! ---> '+profile,color=(1,1,1), clients=[clID], transient=True)
              

        if fire.authentication:
         if profile in MID.authentication:
            matching_values = [prof for prof, value in MID.authentication.items() if value == MID.authentication[profile]]
            matching_profiles = [prof for prof, value in MID.authentication.items() if value == MID.authentication[profile]]
            account = MID.authentication
            value = MID.authentication[profile]

            if value in account.values():
                if len(matching_values) > 1:
                    print('Access Denied: Authentication Sharing Value --> ' + profile)
                    _remove_profile_authentication(account=profile)
                    _log_authentication(account=profile, name=profile_name)
                    bs.screenMessage('Access Denied: Authentication Sharing Value --> '+profile,color=(1,1,1), clients=[clID], transient=True)
                    _kicker(client_id=clID, authentication=True)
                elif len(matching_profiles) > 1:
                    print('Access Denied: Shared Profile')
                else:
                    print('Access Granted: Authentication!')
            else:
                print('Access Denied!')
         else:
            #
            _add_profile_authentication(account=profile)
            if profile not in MID.verification:
                #
                _add_profile_verification(account=profile, add=True)
                
        if fire.authentication:
         if profile in MID.verification:
            if MID.verification.count(profile) == 2:
                print("Access Denied: Verification Sharing Profile")
                bs.screenMessage('Access Denied: Verification Sharing Profile --> '+profile,color=(1,1,1), clients=[clID], transient=True)
                _add_profile_verification(account=profile, add=False)
                _log_verification(account=profile, name=profile_name)
                _kicker(client_id=clID, authentication=False)
                #
                if profile in MID.authentication:
                    #if verification profile, also in the authentication profile, remove it, we dont need him
                    _remove_profile_authentication(account=profile)
                    _kicker(client_id=clID, authentication=False)
            else:
                print("Acess Granted: Verification")
         else:
            if profile not in MID.verification:
                _add_profile_verification(account=profile, add=True)
                #
        ...
        '''
            ...
            if True:
                updated_admins=[]
                updated_admins=MID.authentication
                #     
                MID.authentication[profile] = numbers
                with open(bs.getEnvironment()['systemScriptsDirectory'] + "/membersID.py") as file:
                    s = [row for row in file]              
                    s[46] = 'authentication = '+ str(updated_admins)+ '\n'
                    f = open(bs.getEnvironment()['systemScriptsDirectory'] + "/membersID.py",'w')
                    for i in s:
                        f.write(i)
                    f.close()
                    reload(MID)
                print('Profile has been added! ---> '+profile)
        '''

        '''#if we want to add but looks bad so i have it turned off
        if profile in MID.owners:
            bs.screenMessage('Owners has Joined the Match')

        if profile in MID.mods:
            bs.screenMessage('Admins has Joined the Match')

        if profile in MID.vips:
            bs.screenMessage('VIPs has Joined the Match')

        if profile in MID.members:
            bs.screenMessage('Server Members has Joined the Match')

        
        '''
        clID = self._player.getInputDevice().getClientID()
        if profile not in MID.members_count:
            MID.members_count.append(profile)
            mid_storage.save(MID, {'members_count': MID.members_count})
            

        if profile in MID.rejected:#by PCModder
            clID = self._player.getInputDevice().getClientID()
            name = self._player.getName()
            playeraccountid = profile
            def wow():
                bsInternal._disconnectClient(clID)
            bs.gameTimer(1000,bs.Call(wow))
            #
            ...
            #
            print('Banned Player '+self._player.getName()+' trying to join server Auto Kick = Success')
            print(self._player.getName()+' Unique ID == '+playeraccountid)
            import socket
            ...
            import platform
            f = open("banLog.txt", "a")
            f.write('Banned Player '+self._player.getName()+' trying to join server. Auto Kick = Success\n')
            f.write(self._player.getName()+' Unique ID == '+playeraccountid+'\n')
            f.write('Time Joined == '+now.strftime("%Y-%m-%d %H:%M:%S"+'\n'))
