...
class TeamBaseSession(bs.Session):
    ...
    def __init__(self):
        ...
        if not playlistResolved:
            # Graceful fallback: shared/custom playlist may reference missing
            # mod game types; don't crash the whole host session.
            print 'WARNING: playlist contains no valid games; falling back to default playlist.'
            if self._useTeams:
                playlist = bsUtils._getDefaultTeamsPlaylist()
            else:
                playlist = bsUtils._getDefaultFreeForAllPlaylist()
            playlistResolved = bsUtils._filterPlaylist(
                playlist, sessionType=type(self), addResolvedType=True)

            if not playlistResolved:
                raise Exception("playlist contains no valid games (default fallback also empty)")
