...
class CoopScoreScreen(bs.Activity):
    ...
    def _goToJoiningIfStillEmpty(self):
        try:
            if self.isFinalized():
                return
            if len(self.players) == 0:
                self.getSession().setActivity(bs.newActivity(CoopJoiningActivity))
        except Exception:
            pass

    def _playerPress(self):
        # Headless: allow any action button on score screen to continue,
        # but enforce a minimal view time so players can't skip settlement
        # immediately by accidental early keypress.
        now_ms = bs.getRealTime()
        birth_ms = getattr(self, '_scoreScreenBirthMs', now_ms)
        min_view_ms = getattr(self, '_scoreScreenMinViewMs', 4500)
        if now_ms - birth_ms < min_view_ms:
            return

        if getattr(self, '_continueTriggered', False):
            return
        self._continueTriggered = True
        try:
            self._campaign.setSelectedLevel(self._level)
        except Exception:
            pass
    ...
    def onPlayerJoin(self, player):
        ...
        try:
            if bs.getEnvironment().get('subplatform') == 'headless':
                bs.gameTimer(300, bs.WeakCall(self._safeAssign, player))
        except Exception:
            pass
    ...
    def onBegin(self):
        ...
        # Headless: bind continue keys on score screen for existing players.
        try:
            if bs.getEnvironment().get('subplatform') == 'headless':
                self._continueTriggered = False
                self._scoreScreenBirthMs = bs.getRealTime()
                self._scoreScreenMinViewMs = 4500
                for p in self.players:
                    bs.gameTimer(300, bs.WeakCall(self._safeAssign, p))
        except Exception:
            pass

        # If everyone left mid-round, don't hang here forever.
        # Bounce to joining so new players can press punch to start next run.
        if len(self.players) == 0:
            bs.gameTimer(1500, bs.WeakCall(self._goToJoiningIfStillEmpty))
...
class CoopSession(bs.Session):
    ...
    def _endSessionIfEmpty(self):
        ...
        # If no one is currently in-game but there are session players waiting,
        # go to joining and wait for punch to start (FFA-like).
        if len(self.players) > 0:
        ...
        # No players anywhere.
        # In headless mode, end only the current game to trigger score flow,
        # then go to joining; do not kill whole session.
        if bs.getEnvironment().get('subplatform') == 'headless':
            if isinstance(activity, bs.GameActivity):
                with bs.Context(activity):
                    try:
                        ...
                        activity.end(results={'outcome': 'defeat'})
            else:
                self.setActivity(bs.newActivity(CoopJoiningActivity))
        else:
            self.end()
    ...
    def onActivityEnd(self, activity, results):
        ...
        # Keep session alive even if players drop; let normal score flow run.
        # We'll bounce back to joining after score/end handling.
        ...
        if (isinstance(activity, bsGame.JoiningActivity)
            ...
            # If coming from score-screen with no players, wait in joining.
            # If players press continue on score-screen (outcome=restart),
            # fall through and launch the round immediately.
            if isinstance(activity, CoopScoreScreen) and len(self.players) == 0:
                self.setActivity(bs.newActivity(CoopJoiningActivity))
                return
