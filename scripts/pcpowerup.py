...
import bsInternal
import dist_modded, dist_regular, dist_vanilla
...
_POWERUP_NAME_ZH = {
    'tripleBombs': '三连炸弹', 'punch': '拳套', 'iceBombs': '冰冻炸弹',
    'impactBombs': '感应炸弹', 'landMines': '地雷', 'stickyBombs': '黏弹',
    'shield': '护盾', 'health': '治疗', 'heal': '治疗', 'curse': '诅咒',
    'jumpFly': '跳跃飞行', 'flyer': '飞行器',
    'buffDefenseTemp': '临时防御', 'buffStrengthTemp': '临时力量',
    'buffWingsTemp': '临时翅膀', 'buffLifestealTemp': '临时吸血',
    'buffDefensePerm': '回合防御', 'buffStrengthPerm': '回合力量',
    'buffWingsPerm': '回合翅膀', 'buffLifestealPerm': '回合吸血',
    'antiGrav': '反重力弹', 'boomBomb': '爆裂弹', 'blastBomb': '冲击弹',
    'curseBomb': '诅咒弹', 'iceImpact': '冰感弹', 'knockBomb': '击倒弹',
    'weedbomb': '迷雾弹', 'gluebomb': '胶水弹', 'spazBomb': '角色弹',
    'frozenBomb': '冻结弹', 'portalBomb': '传送门弹', 'teleBomb': '传送弹',
    'fireBomb': '火焰弹', 'powerup': '道具弹', 'iceMine': '冰雷',
    'stickyIce': '黏冰弹', 'shockWave': '冲击波', 'spikeBomb': '尖刺弹',
    'epicMine': '史诗地雷', 'sloMo': '慢动作', 'speedBoots': '加速靴',
    'blackHole': '黑洞', 'mag': '磁力场', 'radius': '范围增强',
    'night': '黑夜模式', 'tnt': 'TNT', 'characterPicker': '角色选择器',
    'colorPicker': '颜色选择器', 'botSpawner': '机器人生成器',
    'blastBot': '爆破机器人', 'bomber': '轰炸机', 'pirateBot': '海盗机器人',
    'beachBall': '沙滩球', 'plsTouchMe': '碰我试试', 'impactShower': '感应炸弹雨',
    'touchShower': '感触炸弹雨', 'pwpShower': '道具雨', 'normalShower': '普通炸弹雨',
    'stickyShower': '黏弹雨', 'iceShower': '冰冻炸弹雨', 'frozenShower': '冻结雨',
    'curseShower': '诅咒雨'
}
...
def getDefaultPowerupDistribution():#finish up
    if set_pwp.modded_powerups:
        if set_pwp.vanilla == True:
            return dist_vanilla.vanilla_dist
        else:
            return dist_modded.powerup_dist
    elif set_pwp.vanilla:
        if set_pwp.modded_powerups == True:
            return dist_modded.powerup_dist
        else:
            return dist_vanilla.vanilla_dist
    else:
        return dist_regular.regular_dist
...
class Powerup(bs.Actor):#PCModder
    ...
    def __init__(self,position=(0,1,0),powerupType='tripleBombs',expire=True):
        ...
        elif powerupType == 'buffDefenseTemp': tex = factory.texShield
        elif powerupType == 'buffStrengthTemp': tex = factory.texPunch
        elif powerupType == 'buffWingsTemp': tex = factory.texJumpFly
        elif powerupType == 'buffLifestealTemp': tex = factory.texHealth
        elif powerupType == 'buffDefensePerm': tex = factory.texShield
        elif powerupType == 'buffStrengthPerm': tex = factory.texSpunch
        elif powerupType == 'buffWingsPerm': tex = factory.texJumpFly
        elif powerupType == 'buffLifestealPerm': tex = factory.texHealth
        ...
        if set_pwp.powSplint:
            self._inv = bs.Timer(100, bs.Call(lambda self: bs.emitBGDynamics(
                    position = self.node.position,
                    count = 20, chunkType='splinter',
                    scale=0.55, spread=0.6) if self.node.exists() else None, self), repeat = True)
                    
        if set_pwp.powSlime:#changed it to spark actually slime looks useless
            self._inv = bs.Timer(100, bs.Call(lambda self: bs.emitBGDynamics(
                    position = self.node.position,
        ...
        if expire:
            ...
            self._startExpirationTimer()  # Start the expiration timer for this power-up.

    def _startExpirationTimer(self):
        # Start a timer for this power-up.
        self._expirationTime = defaultPowerupInterval / 1000.0  # Convert to seconds
        bs.gameTimer(1000, self._updateExpirationTimer, repeat=True)

    def _updateExpirationTimer(self):
        # Update the expiration timer for this power-up.
        if set_pwp.powerup_text:
         if self.node.exists() and self._expire:
            self._expirationTime -= 1.0
            if self._expirationTime > 0.0:
                # Display a screen message showing the remaining time above the power-up.
                #bs.screenMessage("Time left: " + str(int(self._expirationTime)), color=(0, 1, 0
                bsUtils.PopupText(str(max(int(self._expirationTime), 0)),color=(1,1,1),
                                                         scale=0.9,
                                                         offset=(0,1.5,0),
                                                         position=(self.node.position[0],self.node.position[1]-1,self.node.position[2])).autoRetain()
                    ...
                    if self.powerupType == "volcano":
                        ...
                        if self.node.position in objs.lastpos:
                            self.portal = objs.Portal(position1=None, r=0.9,
                                                      ...
                                                      activity=bs.getActivity())
                            bs.gameTimer(8000, bs.Call(self.delete_portal))
                        else:
                        ...
                        self._powersGiven = True
                        self.handleMessage(bs.DieMessage())
                        bsUtils.PopupText("TheGreat",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()
                        ...
                        def checkVal(val):
                            ...
                            if val and spaz.isAlive():
                               ...
                               bsUtils.PopupText("MythB",color=(1,2,1),scale=1.0,position=self.node.position).autoRetain()
                        ...
                        bsUtils.PopupText("Froshlee14",color=(1,2,1),scale=1.0,position=self.node.position).autoRetain()
                        ...
                        #bsUtils.PopupText("TNT",color=(1,2,1),scale=1.0,position=self.node.position).autoRetain()
                       ...
                       # bsUtils.PopupText("TNT",color=(1,2,1),scale=1.0,position=self.node.position).autoRetain()
                        ...
                        bs.Bomb((p[0]+0.43,p[1]+4,p[2]-0.25),velocity=(0,-6,0),bombType = 'ice').autoRetain() 
                        bs.Bomb((p[0]+0.43,p[1]+4,p[2]-0.25),velocity=(0,-6,0),bombType = 'ice').autoRetain()     
                        #bsUtils.PopupText("ICY",color=(1,2,1),scale=1.0,position=self.node.position).autoRetain()
                        ...
                        if event in [1]:                        
                            ...
                            node.style = 'frosty'       
                            bsUtils.PopupText("Frosty The Snowman",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain() 
                        elif event in [2]:         
                            ...
                            node.style = 'wizard'
                            bsUtils.PopupText("EVIL SCEPTER WIZARD MAN",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()   
                        elif event in [4]:           
                            ...
                            node.style = 'pixie'
                            bsUtils.PopupText("PIXIEL-ATED",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()   
                        elif event in [5]:           
                            ...
                            node.style = 'cyborg'
                            bsUtils.PopupText("The Robo",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()    
                        elif event in [6]:    
                            ...
                            bsUtils.PopupText("PC||Modder",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()                             
                        ...
                        node.toesModel = None      
                        #bsUtils.PopupText("Invisible",color=(1,2,1),scale=1.0,position=self.node.position).autoRetain()                                
                        ...
                        #bsUtils.PopupText("TRoLL",color=(1,2,1),scale=1.0,position=self.node.position).autoRetain()
                        ...
                        #bsUtils.PopupText("Champ",color=(1,2,1),scale=1.0,position=self.node.position).autoRetain()
                    else:
...
# keep bs namespace in sync for code that calls bs.Powerup directly
bs.Powerup = Powerup