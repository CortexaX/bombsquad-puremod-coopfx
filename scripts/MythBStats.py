...
def update(score_set):
    ...
    # Avoid unnecessary background threads for non-game activities.
    if len(account_kills) == 0:
        return

    # Now kick off a background thread to avoid hitches.
    UpdateThread(account_kills, account_killed, account_scores,
                 account_played, account_name, account_localname).start()
    ...
    def run(self):
        ...
        for account_id, kill_count in self._account_kills.items():
            ...
            if account_id not in stats:
                ...
                try:
                    response = json.loads(
                        urllib2.urlopen(urllib2.Request(url), timeout=5).read())
                    name_html = response.get('name_html', self._account_localname.get(account_id, account_id))
                except Exception:
                    name_html = self._account_localname.get(account_id, account_id)
