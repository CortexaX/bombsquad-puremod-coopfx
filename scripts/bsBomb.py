...
class Bomb(bs.Actor):
    ...
    def __init__(self, position=(0,1,0), velocity=(0,0,0), bombType='normal',
        ...
        if self.bombType not in ('landMine','tnt'):
            ...
            if self.bombType == 'ice':
                self.shield = bs.newNode('shield', owner=self.node,#PC||Modder was here
                attrs={'color':(1,3,12),'radius':0.6})
    ...
    def _handleImpact(self,m):
        ...
        if node is not None and node.exists():
            if self.bombType == 'impact':
                ...
                # impact bomb should NOT explode only when touching real players.
                # Bots/spazbots should still detonate impact bombs.
                try:
                    import bsSpaz
    ...
    def _handleHit(self, msg):
        ...
        # impact/landmine also ignore punch hits (boxing gloves shouldn't detonate)
        if (not self._exploded and not isPunch
            and self.bombType in ['normal', 'sticky', 'impact', 'landMine', 'ice', 'tnt']):
