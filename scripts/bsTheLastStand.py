...
class TheLastStandGame(bs.CoopGameActivity):
    ...
    def _startBotUpdates(self):
        ...
        # Stagger opening waves so enemies don't flood in all at once.
        self._updateBots()
        bs.gameTimer(2200, bs.WeakCall(self._updateBots))
        if len(self.players) > 2:
            bs.gameTimer(3600, bs.WeakCall(self._updateBots))
        if len(self.players) > 3:
            bs.gameTimer(4800, bs.WeakCall(self._updateBots))
