...
_mod_bootstrap = None
_mod_bootstrap_loaded = False


def _ensure_mod_bootstrap_loaded():
    """Load mod bootstrap lazily once runtime is fully initialized."""
    global _mod_bootstrap
    global _mod_bootstrap_loaded

    if _mod_bootstrap_loaded:
        return

    try:
        if _mod_bootstrap is None:
            import mod_bootstrap as _mb
            _mod_bootstrap = _mb
        _mod_bootstrap.bootstrap()
    except Exception:
        # Avoid relying on bs.printException during import-order edge cases.
        try:
            import traceback
            traceback.print_exc()
        except Exception:
            pass
    _mod_bootstrap_loaded = True
...
class Activity(object):
    ...
    def onBegin(self):
        ...
        """
        _ensure_mod_bootstrap_loaded()

        if MythBStats is not None:
            try:
                MythBStats.update(self.scoreSet)
            except Exception:
                try:
                    bs.printException('MythBStats.update failed')
                except Exception:
                    try:
                        import traceback
                        traceback.print_exc()
