...
_POWERUP_NAME_ZH = {
    'tripleBombs': '三连炸弹', 'triple': '三连炸弹',
    'iceBombs': '冰冻炸弹', 'impactBombs': '感应炸弹', 'landMines': '地雷',
    'stickyBombs': '黏弹', 'stickyShower': '黏弹雨', 'normalShower': '普通炸弹雨',
    'shield': '护盾', 'heal': '治疗', 'health': '治疗', 'healt': '治疗',
    'punch': '拳套', 'punchs': '拳套',
    'invincible': '无敌', 'speedBoots': '加速靴',
    'buffDefenseTemp': '临时防御', 'buffDefensePerm': '回合防御',
    'buffStrengthTemp': '临时力量', 'buffStrengthPerm': '回合力量',
    'buffWingsTemp': '临时天使之翼', 'buffWingsPerm': '回合天使之翼',
    'buffLifestealTemp': '临时吸血', 'buffLifestealPerm': '回合吸血',
    'jumpFly': '跳跃飞行', 'flyer': '飞行器',
    'antiGrav': '反重力弹', 'impactShower': '感应炸弹雨',
    'iceShower': '冰冻炸弹雨', 'frozenShower': '冰冻雨',
    'touchShower': '感触炸弹雨', 'pwpShower': '道具雨',
    'boomBomb': '爆裂弹', 'blastBomb': '冲击弹', 'curseBomb': '诅咒弹',
    'cursyBomb': '诅咒弹', 'iceImpact': '冰感弹', 'spikeBomb': '尖刺弹',
    'spazBomb': '角色弹', 'fireBomb': '火焰弹', 'teleBomb': '传送弹',
    'portalBomb': '传送门弹', 'weedbomb': '迷雾弹', 'gluebomb': '胶水弹',
    'knockBomb': '击倒弹', 'hybridBomb': '混合弹', 'frozenBomb': '冻结弹',
    'epicMine': '史诗地雷', 'iceMine': '冰雷', 'tnt': 'TNT',
    'blackHole': '黑洞', 'shockWave': '冲击波', 'shock': '电击',
    'sloMo': '慢动作', 'radius': '范围增强', 'mag': '磁力场',
    'characterPicker': '角色选择器', 'colorPicker': '颜色选择器',
    'botSpawner': '机器人生成器', 'blastBot': '爆破机器人', 'pirateBot': '海盗机器人',
    'beachBall': '沙滩球', 'bomber': '轰炸机', 'plsTouchMe': '碰我试试',
    'night': '黑夜模式', 'light': '闪电', 'boom': '爆破',
    'curse': '诅咒', 'cursy': '诅咒', 'freeze': '冻结',
    'rchar': '随机角色', 'rchar1': '随机角色', 'rcolor': '随机颜色',
    'frosty': '雪人', 'wizard': '巫师', 'ali': '拳王阿里', 'ninja': '忍者',
    'santa': '圣诞老人', 'pengu': '企鹅', 'pixie': '小精灵', 'robot': '机器人',
}


def _powerup_name_zh(powerup_type):
    return _POWERUP_NAME_ZH.get(powerup_type, powerup_type)
...
def newSpazInit(self, *args, **kwargs):
    ...
    # Buff system (temporary + per-round permanent)
    self._buffDefenseTemp = False
    self._buffDefensePerm = False
    self._buffStrengthTemp = False
    self._buffStrengthPerm = False
    self._buffWingsTemp = False
    self._buffWingsPerm = False
    self._buffLifestealTemp = False
    self._buffLifestealPerm = False

    self._basePunchPowerScale = self._punchPowerScale
    self._basePunchCooldown = self._punchCooldown
    ...
    if set_spaz.defaultBoxingGloves and isinstance(self, PlayerSpaz): self.equipBoxingGloves()
    if set_spaz.defaultShields: self.equipShields()
    #for default char
    if set_spaz.rchar: self.node.handleMessage(bs.PowerupMessage(powerupType = 'rchar1'))
    if set_spaz.frosty: self.node.handleMessage(bs.PowerupMessage(powerupType = 'frosty'))
    if set_spaz.wizard: self.node.handleMessage(bs.PowerupMessage(powerupType = 'wizard'))
    if set_spaz.ali: self.node.handleMessage(bs.PowerupMessage(powerupType = 'ali'))
    if set_spaz.santa: self.node.handleMessage(bs.PowerupMessage(powerupType = 'santa'))
    if set_spaz.robot: self.node.handleMessage(bs.PowerupMessage(powerupType = 'robot'))
    if set_spaz.pengu: self.node.handleMessage(bs.PowerupMessage(powerupType = 'pengu'))
    if set_spaz.pixie: self.node.handleMessage(bs.PowerupMessage(powerupType = 'pixie'))
    if set_spaz.ninja: self.node.handleMessage(bs.PowerupMessage(powerupType = 'ninja'))
    #for default bombs
    if set_spaz.impact_bomb: self.bombType = 'impact'
    if set_spaz.sticky_bomb: self.bombType = 'sticky'
    if set_spaz.ice_bomb: self.bombType = 'ice'
    if set_spaz.spike_bomb: self.bombType = 'spikeBomb'
    if set_spaz.shock_wave: self.bombType = 'shockWave'
    if set_spaz.knock_bomb: self.bombType = 'knockBomb'
    if set_spaz.spaz_bomb: self.bombType = 'spazBomb'
    if set_spaz.glue_bomb: self.bombType = 'gluebomb'
    
    prefix = random.choice([u'\ue048THE BOSS\ue048',u'\ue00cNOOBY\ue00c',u'\ue00cTERMINATOR\ue00c',u'\ue043不错\ue043',u'\ue043很棒\ue043',u'\ue048无敌了孩子\ue048'])
    ...
    if set_spaz.tag: self._pcText.text = prefix
...
def onJumpfly(self):
        """
        Legacy boosted-jump mode kept for compatibility.
        """
...
def onJumpFlyNative(self):
        """
        Native-like wings behavior:
        - jump key still triggers normal jump
        - same press also enables fly thrust
        """
...
def onJumpFlyNativeRelease(self):
        """
        Release handler for native-like wings behavior.
        """
        if not self.node.exists():
            return
        try:
...
def _refreshBuffStats(self):
        # Defense scales incoming impulse/damage (handled in HitMessage)
        # Strength boosts punch output
        strength = 1.0
        cooldown = self._basePunchCooldown
        if self._buffStrengthTemp:
            strength += 0.45
            cooldown = min(cooldown, 260)
        if self._buffStrengthPerm:
            strength += 0.70
            cooldown = min(cooldown, 220)
        self._punchPowerScale = self._basePunchPowerScale * strength
        self._punchCooldown = cooldown


def _setAngelWingsInput(self, enable):
        try:
        ...
        try:
            if enable:
                # Native wings: jump press/release drive both jump + fly.
                player.assignInputCall('jumpPress', self.onJumpFlyNative)
                player.assignInputCall('jumpRelease', self.onJumpFlyNativeRelease)
                # Important: fly thrust only works when node.fly is enabled.
                self.node.fly = True
            else:
...
def _tempBuffOff(self, name):
        ...
        if name == 'defense':
            ...
            self._buffWingsTemp = False
            if not self._buffWingsPerm:
                _setAngelWingsInput(self, False)
        elif name == 'lifesteal':
            self._buffLifestealTemp = False
        _refreshBuffStats(self)
        bsUtils.PopupText(name + ' temp off', color=(1,1,1), scale=0.9, position=self.node.position).autoRetain()
...
def pchandleMessage(self, msg):
        ...
        if isinstance(msg, bs.PickedUpMessage):
            ...
            if set_spaz.nameP:
                bsUtils.PopupText(_powerup_name_zh(msg.powerupType) + "!",
                             color=(1, 1, 1),
                             scale=1.0,
                             position=self.node.position).autoRetain()#thanks to logic
                
            if set_pwp.lightning: # #light node done by me concept by esie-eyen and patronmodz
            ...
            if msg.powerupType == 'buffDefenseTemp':
                self._buffDefenseTemp = True
                _refreshBuffStats(self)
                bsUtils.PopupText('Defense+ (Temp)', color=(0.6,0.9,1), scale=1.0, position=self.node.position).autoRetain()
                bs.gameTimer(15000, bs.Call(_tempBuffOff, self, 'defense'))

            elif msg.powerupType == 'buffStrengthTemp':
                self._buffStrengthTemp = True
                _refreshBuffStats(self)
                bsUtils.PopupText('Strength+ (Temp)', color=(1,0.8,0.5), scale=1.0, position=self.node.position).autoRetain()
                bs.gameTimer(15000, bs.Call(_tempBuffOff, self, 'strength'))

            elif msg.powerupType == 'buffWingsTemp':
                self._buffWingsTemp = True
                _setAngelWingsInput(self, True)
                bsUtils.PopupText('Angel Wings (Temp)', color=(0.8,0.95,1), scale=1.0, position=self.node.position).autoRetain()
                bs.gameTimer(15000, bs.Call(_tempBuffOff, self, 'wings'))

            elif msg.powerupType == 'buffLifestealTemp':
                self._buffLifestealTemp = True
                bsUtils.PopupText('Lifesteal (Temp)', color=(1,0.6,0.6), scale=1.0, position=self.node.position).autoRetain()
                bs.gameTimer(15000, bs.Call(_tempBuffOff, self, 'lifesteal'))

            elif msg.powerupType == 'buffDefensePerm':
                self._buffDefensePerm = True
                _refreshBuffStats(self)
                bsUtils.PopupText('Defense+ (Round)', color=(0.6,0.9,1), scale=1.0, position=self.node.position).autoRetain()

            elif msg.powerupType == 'buffStrengthPerm':
                self._buffStrengthPerm = True
                _refreshBuffStats(self)
                bsUtils.PopupText('Strength+ (Round)', color=(1,0.8,0.5), scale=1.0, position=self.node.position).autoRetain()

            elif msg.powerupType == 'buffWingsPerm':
                self._buffWingsPerm = True
                _setAngelWingsInput(self, True)
                bsUtils.PopupText('Angel Wings (Round)', color=(0.8,0.95,1), scale=1.0, position=self.node.position).autoRetain()

            elif msg.powerupType == 'buffLifestealPerm':
                self._buffLifestealPerm = True
                bsUtils.PopupText('Lifesteal (Round)', color=(1,0.6,0.6), scale=1.0, position=self.node.position).autoRetain()

            elif (msg.powerupType == 'tripleBombs'):
                ...
                self.setfireBombCount(min(self.fireBombCount+3, 3))
            elif msg.powerupType == 'shockWave':
                self.setshockWaveCount(min(self.shockWaveCount+3, 3))
                ...
                bsUtils.PopupText("Esie-Eyen",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()
                self._blast = True                
                if self._blast == True:
                ...
                bs.gameTimer(2000,bs.Call(weed))
                bs.gameTimer(5500,bs.Call(weed))
                bs.gameTimer(8500,bs.Call(weed))
                def hiccups():
                    if self.isAlive():
                        node = self.node
                        bs.emitBGDynamics(position=(node.position[0],node.position[1]-1.2,node.position[2]), velocity=(0,0.05,0), count=random.randrange(100,270), scale=1+random.random(), spread=0.71, chunkType='sweat')
                bs.gameTimer(1000,bs.Call(hiccups))
                bs.gameTimer(2500,bs.Call(hiccups))
                ...
                bs.gameTimer(1460,bs.Call(looks))
                bs.gameTimer(2960,bs.Call(looks))
                bs.gameTimer(5460,bs.Call(looks))
                bs.gameTimer(7960,bs.Call(looks))
            elif msg.powerupType == 'frozenBomb':
                self.bombType = 'frozenBomb'
                tex = self._getBombTypeTex()
                self._flashBillboard(tex)
                if set_pwp.lightning:
                    self.light = bs.newNode('light',
                    attrs={'position':self.node.position,
                           'color':(1.2,1.2,1.4),
                           'volumeIntensityScale': 2.35})
                ...
                bsUtils.PopupText("Dimitry",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()
            elif msg.powerupType == 'radius':
                self.blastRadius = 5.5
                def radius():
                    if self.isAlive():
                        self.blastRadius = 2.0
                        bsUtils.PopupText("Radius turned Off!",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()
                bs.gameTimer(8000,bs.Call(radius))
                ...
                bsUtils.PopupText("SobyDamn",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()
            ...
            elif msg.powerupType =='speedBoots':    
                self.node.hockey = True              
                def _speed():
                    if self.isAlive():
                        self.node.hockey = False
                        bsUtils.PopupText('Speed turned Off!',color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()
                bs.gameTimer(5000,bs.Call(_speed)) 
                ...
                def _speed():
                    if self.isAlive():
                        ...
                        bsUtils.PopupText('Inv turned Off!',color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()
                bs.gameTimer(5000,bs.Call(_speed))
                ...
                bsUtils.PopupText("Froshlee14",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()
                if self.powerupsExpire:
                    self.node.miniBillboard3Texture = tex
                    t = bs.getGameTime()
                    self.node.miniBillboard3StartTime = t
            ...
            #     bs.shakeCamera(5)
                ...
                bs.screenMessage('%s called in a drone strike!' %
                                 ...
                                 color=(1, 1, 1))
                bsUtils.PopupText("BombDash/Logic",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()
            # bs.gameTimer(500,strike)
                ...
                if event in [1]:
                    ...
                    self.node.toesModel = bs.getModel('frostyToes')
                    self.node.style = 'frosty'    
                    bsUtils.PopupText("FrostY",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()                    
                    ...
                    self.node.toesModel = bs.getModel('santaToes')
                    self.node.style = 'santa'
                    bsUtils.PopupText("Santa",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()
                    ...
                    self.node.toesModel = bs.getModel('bonesToes')
                    self.node.style = 'bones'
                    bsUtils.PopupText("Bones",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()
                    ...
                    self.node.toesModel = bs.getModel('melToes')
                    self.node.style = 'mel'
                    bsUtils.PopupText("Chef Mel",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()
                    ...
                    self.node.toesModel = bs.getModel('ninjaToes')
                    self.node.style = 'ninja'
                    bsUtils.PopupText("Secret Ninja",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()
                    ...
                    self.node.toesModel = bs.getModel('aliToes')
                    self.node.style = 'ali'
                    bsUtils.PopupText("ALI",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()
                    ...
                    self.node.toesModel = bs.getModel('bearToes')
                    self.node.style = 'bear'
                    bsUtils.PopupText("BEAR",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()
                    ...
                    self.node.toesModel = bs.getModel('penguinToes')
                    self.node.style = 'penguin'
                    bsUtils.PopupText("Pengu",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()
                    ...
                    self.node.toesModel = bs.getModel('wizardToes')
                    self.node.style = 'spaz'
                    bsUtils.PopupText("Spazy",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()
                    ...
                    self.node.toesModel = bs.getModel('pixieToes')
                    self.node.style = 'pixie'
                    bsUtils.PopupText("Pixie",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()
                    ...
                    self.node.toesModel = bs.getModel('cyborgToes')
                    self.node.style = 'cyborg'
                    bsUtils.PopupText("Robot",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()
                    
            elif msg.powerupType == 'rchar1':
                ...
                def _bruh():
                    ...
                    event = random.randint(1,9) if testingEvent == 0 else testingEvent                    
                    
                    if event in [1]:
                        self.node.handleMessage(bs.PowerupMessage(powerupType='boomBomb'))
                        bsUtils.PopupText("BoomBomb",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()                    
                    
                    elif event == 2:  
                        self.node.handleMessage(bs.PowerupMessage(powerupType='blackHole'))
                        bsUtils.PopupText("BlackHole",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()

                    elif event == 3:     
                        self.node.handleMessage(bs.PowerupMessage(powerupType='frozenBomb'))
                        bsUtils.PopupText("FrozenBomb",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()
                    
                    elif event == 4:      
                        self.node.handleMessage(bs.PowerupMessage(powerupType='curse'))
                        bsUtils.PopupText("Curse",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()
                    
                    elif event == 5:    
                        self.node.handleMessage(bs.PowerupMessage(powerupType='health'))
                        bsUtils.PopupText("Health",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()

                    elif event == 6:           
                        self.node.handleMessage(bs.PowerupMessage(powerupType='punch'))
                        bsUtils.PopupText("Gloves",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()

                    elif event == 7:           
                        self.node.handleMessage(bs.PowerupMessage(powerupType='shield'))
                        bsUtils.PopupText("Shield",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()
                    
                    elif event == 8:           
                        self.node.handleMessage(bs.PowerupMessage(powerupType='spunch'))
                        bsUtils.PopupText("Spunch",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()

                    elif event == 9:           
                        self.node.handleMessage(bs.PowerupMessage(powerupType='knockBomb'))
                        bsUtils.PopupText("Knock",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()
                ...
                tex = self._getBombTypeTex()
                self._flashBillboard(tex)
                bsUtils.PopupText("SobyDam",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()
                if self.powerupsExpire:
                    self.node.miniBillboard2Texture = tex
                    t = bs.getGameTime()
                    self.node.miniBillboard2StartTime = t
                ...
                tex = self._getBombTypeTex()
                self._flashBillboard(tex)
                bsUtils.PopupText("ShayPlays",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()
                if self.powerupsExpire:
                    self.node.miniBillboard2Texture = tex
                    t = bs.getGameTime()
                    self.node.miniBillboard2StartTime = t
                ...
                bsUtils.PopupText("BombDash",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()
                ...
                bsUtils.PopupText("4 Sec to Revenge!",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()
                def _boom():
                    if self.isAlive():
                        self.node.handleMessage(bs.DieMessage())
                        bsUtils.PopupText(u"Revenged!",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()
                ...
                def _boom():
                    self.node.handleMessage(bs.PowerupMessage(powerupType = 'heal'))
                    bsUtils.PopupText(u"Healed up!",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()
                ...
                self.curse()
            elif msg.powerupType == 'curs':
                self.curse()
                bsUtils.PopupText(u"Bye!",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()
            elif msg.powerupType == 'sloMo':
                bs.getSharedObject('globals').slowMotion = bs.getSharedObject('globals').slowMotion == False
                bsUtils.PopupText(u"\ue00eSLO-MO!\ue00e",color=(1,0.5,0),scale=1.0,position=self.node.position).autoRetain()
                bs.emitBGDynamics(position=self.node.position,velocity=(0,0,0),count=600,spread=0.7,chunkType='ice');
                ...
                self.bombType = 'ice'
                tex = self._getBombTypeTex()
                bsUtils.PopupText("IcyBombs", color = self.node.color,scale = 1.0, position = self.node.position).autoRetain()
                self._flashBillboard(tex)
                if self.powerupsExpire:
                    self.node.miniBillboard2Texture = tex
                    t = bs.getGameTime()
                ...
                self._numTimesHit = 0
            elif (msg.powerupType == 'freeze'):
                self.node.handleMessage(bs.FreezeMessage())
                bsUtils.PopupText(u"Frozen!",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()
            elif (msg.powerupType == 'healt'):
                if self._cursed:
                    self._cursed = False
                    # remove cursed material
                    factory = self.getFactory()
                ...
                self._lastHitTime = None
                self._numTimesHit = 0
                bsUtils.PopupText(u"Health!",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()
            ...
            # Defense buff reduces incoming impulse/damage
            defense_mul = 1.0
            if self._buffDefenseTemp:
                defense_mul *= 0.75
            if self._buffDefensePerm:
                defense_mul *= 0.65
            mag *= defense_mul
            velocityMag *= defense_mul

            damageScale = 0.22 * defense_mul
            ...
            if msg.hitType == 'punch':
                ...
                if damage > 200 and damage < 450:
                    bsUtils.showDamageCount('-' + str(int(damage/10)) + "%", msg.pos, msg.forceDirection)
                 #   bsUtils.PopupText(u"noob",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()
                if damage > 450 and damage < 800:
                    bsUtils.showDamageCount('-' + str(int(damage/10)) + "%", msg.pos, msg.forceDirection)
                    bsUtils.PopupText(u"🤣",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()

                if damage > 801 and damage < 1109:
                    bsUtils.showDamageCount('-' + str(int(damage/10)) + "%", msg.pos, msg.forceDirection)
                    bsUtils.PopupText(u"\ue047close\ue047",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()
                if damage > 1110 and damage < 1500:
                    bsUtils.showDamageCount('-' + str(int(damage/10)) + "%", msg.pos, msg.forceDirection)
                    bsUtils.PopupText(u"\ue048ez\ue048",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()
                if damage > 450:
                    _spark()
                ...
                flashColor = (1.0, 0.8, 0.4)#normal
                if damage > 450:
                    flashColor = (1, random.random()/2, random.random()/2)
            ...
            if set_spaz.hp:
                self._hpText.text = str(int(self.hitPoints / 10)) + ' %'
            else:
                self._hpText.text = ''
            ...
            if (node is not None and node.exists()
                ...
                # Lifesteal buff: heal attacker on successful punch-hit
                if self._buffLifestealTemp or self._buffLifestealPerm:
                    heal = 70 if self._buffLifestealTemp else 0
                    heal += 120 if self._buffLifestealPerm else 0
                    self.hitPoints = min(self.hitPointsMax, self.hitPoints + heal)
                    if self.node.exists():
                        self.node.hurt = max(0.0, self.node.hurt - 0.06)
                    if random.random() < 0.35:
                        bsUtils.PopupText('+HP', color=(1,0.4,0.4), scale=0.8, position=self.node.position).autoRetain()
...
def _jumpFlyWearOff(self):
        ...
        if self.node.exists():
            self.node.fly = False
            self.node.billboardOpacity = 0.0     
            bsUtils.PopupText("JumpFly turned Off!",color=(1,1,1),scale=1.0,position=self.node.position).autoRetain()             
...
Spaz._refreshBuffStats = _refreshBuffStats
Spaz._setAngelWingsInput = _setAngelWingsInput
Spaz._tempBuffOff = _tempBuffOff
Spaz._nightWearOff = _nightWearOff
Spaz._nightWearOffFlash = _nightWearOffFlash
Spaz.onJumpfly = onJumpfly
Spaz.onJumpFlyNative = onJumpFlyNative
Spaz.onJumpFlyNativeRelease = onJumpFlyNativeRelease
